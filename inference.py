...
def run_task(task_index: int, client, baseline_agent: BaselineInterviewAgent) -> dict[str, object]:
    ...
    step_count = int(state.get("step_count", steps))
    cumulative = float(state.get("cumulative_score", 0.0))
    average_score = cumulative / step_count if step_count > 0 else 0.0
    bounded_score = max(EPSILON, min(1.0 - EPSILON, average_score))
    _log_end(task_name, bounded_score, step_count)
    return {
